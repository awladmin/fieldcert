...
import { activateSubscription, createOrg, saveProfileName, startTrial } from "@/actions/orgs";
...
export function OnboardingWizard({
  ...
  function submitTrial() {
    setError(undefined);
    startTransition(async () => {
      const result = await startTrial(accountType === "individual" ? 1 : seats);
      if (result.error) return setError(result.error);
      router.push("/dashboard");
    });
  }
      ...
      <Card className="w-full max-w-lg">
        <CardHeader>
          ...
          <CardTitle>
            ...
            {step === "billing" && (trialExpired ? "Your free trial has ended" : "Start your 30-day free trial")}
          </CardTitle>
          <CardDescription>
            ...
            {step === "billing" &&
              (trialExpired
                ? "Pick your plan to keep issuing certificates. Your data is all still here."
                : "The full product, free for 30 days. No card needed.")}
          </CardDescription>
        </CardHeader>
              ...
              {trialExpired ? (
                ...
                <>
                  <ul className="text-muted-foreground flex flex-col gap-1.5 text-sm">
                    <li>Every feature, including the AI board scanner</li>
                    <li>Unlimited certificates during the trial</li>
                    <li>No card needed. Nothing to cancel</li>
                  </ul>
                  <Button onClick={submitTrial} disabled={pending} className="h-11">
                    {pending ? "Starting" : "Start my 30-day free trial"}
                  </Button>
                  <p className="text-muted-foreground text-xs">
                    After 30 days you pick a plan: £{monthlyTotal.toFixed(2)}/mo + VAT as configured
                    above. We will remind you before it ends.
                  </p>
                </>
              )}
            </>
